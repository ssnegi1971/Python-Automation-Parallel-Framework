...
for index, row in df.iterrows():
    if row.total_cnt != row.cnt_success:
        with open('C:/Users/Admin/Desktop/work/Python/PythonAutomation/order/keepprocessing.txt', 'w') as f:
            print("LOOP", file=f);
    else:
        with open('C:/Users/Admin/Desktop/work/Python/PythonAutomation/order/keepprocessing.txt', 'w') as f:
            print("STOP", file=f);
cursor.commit();
query = "select count(*) RunJobs from dbo.etl_job_runs where jobstatus = 'RUNNING';"
df = pd.read_sql(query, engine);
for index, row in df.iterrows():
    with open('C:/Users/Admin/Desktop/work/Python/PythonAutomation/order/nextjobs.txt', 'w') as f:
